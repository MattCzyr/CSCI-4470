...
def generate_graph(words):
    ...
    def edit_distance_one(word):
        for diff_word in list(permutations(word)):
            new_word = "".join(diff_word)
...
if __name__ == '__main__':
    G = words_graph()
    print("Loaded words_dat.txt containing 5757 five-letter English words.")
    print("Two words are connected if they differ in one letter without regard to ordering.")
    print("Graph has %d nodes with %d edges"
          % (nx.number_of_nodes(G), nx.number_of_edges(G)))
    print("%d connected components" % nx.number_connected_components(G))
...
```
Loaded words_dat.txt containing 5757 five-letter English words.
Two words are connected if they differ in one letter without regard to ordering.
